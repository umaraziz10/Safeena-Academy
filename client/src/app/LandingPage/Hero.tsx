...
const Index = (): JSX.Element => {
  ...
  const [continueCourseLink, setContinueCourseLink] = useState<string>('/Educational/Courses/1');
  useEffect(() => {
    async function fetchContinueCourse() {
      try {
        const courseId = 1; // ID Course, kamu bisa ubah dinamis kalau mau
        const res = await fetchWithToken(`/materials/course/${courseId}`);
        const data = await res.json();

        // Filter yang status === 0 (belum complete)
        const unfinishedMaterials = data.filter((material: any) => material.status === 0);

        if (unfinishedMaterials.length > 0) {
          // Cari material dengan material_id terkecil
          unfinishedMaterials.sort((a: any, b: any) => a.material_id - b.material_id);
          const firstUnfinished = unfinishedMaterials[0];

          setContinueCourseLink(`/Educational/Learning/${firstUnfinished.material_id}`);
        } else {
          // Kalau semua selesai, fallback ke Courses page
            ...
            <div className='max-w-[1070px] mx-auto px-4'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-0 px-2 lg:px-0'>
                {serviceCards.map((card, index) => {
                  ...
                  if (card.title === 'Continue your Courses') link = continueCourseLink;
                  if (card.title === 'View All Courses') link = '/Educational';
                  if (card.title === 'Your Completion Badge') link = '/Badge';
                  if (card.title === '24/7 Support') link = '/Chatbot';
                  ...
                  return (
                    ...
                    >
                      <a
                        href={link}
                        className='w-full max-w-[280px] block rounded-[10px] border-[3px] sm:border-[4px] lg:border-[6px] border-solid shadow-[4px_4px_4px_#00000040] sm:shadow-[7px_6px_4px_#00000040] transition-all duration-300'
                      >
                        <Card
                          className={`
                            w-full max-w-[280px] 
