      ...
      try {
        ...
        rawTimeSlots.forEach((slot: any) => {
          ...
          grouped[period].slots.push({
            id: slot.id,
            time: timeStr,
            available: true, // or adjust based on condition
          });
        });
        ...
        <CardContent className='p-4 sm:p-7'>
          ...
          <section className='mb-6 sm:mb-8'>
            ...
            <input
              type="date"
              value={selectedDate}
              onChange={handleDateChange}  // ✅ gunakan handler yang kamu buat
              ...
              {servicesData.map((serviceType) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {serviceType === 'onsite' ? 'On-site' : serviceType === 'e-counseling' ? 'E-Counseling' : serviceType}
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 text-center animate-fade-in">
            <h2 className="text-xl font-semibold text-[#337bbf] mb-3">
              Penjadwalan Berhadil Dijadwalkan!
            </h2>
            <p className="text-gray-700 mb-6">
              Konsultasi kamu telah berhasil di jadwalkan. Kamu dapat melihat detaiil konsultasi di halaman Psikologis.
            </p>
            <button
              onClick={() => router.push('/Psychologist')}
              className="bg-[#337bbf] text-white px-6 py-2 rounded hover:bg-yellow-500 transition"
            >
              Pergi ke Halaman Psikolog
            </button>
          </div>
        </div>
      )}
