        ...
        <Link href="/" className="relative z-10">
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10 md:w-12 md:h-12 overflow-hidden rounded-full border-2 border-forest-500 z-50 shadow-md">
              <Image src="/Navbar.png" alt="Safeena Academy Logo" width={48} height={48} className="object-cover" />
            </div>
            <div className="flex flex-col">
              <span className="text-lg md:text-xl font-bold text-forest-700 leading-tight">Safeena</span>
              <span className="text-xs md:text-sm text-forest-600 font-medium tracking-wider">ACADEMY</span>
            </div>
          </div>
        </Link>
      ...
      >
        <div className="container bg-white mx-auto px-6 overflow-y-auto max-h-[calc(100vh-5rem)]">
          <ul className="flex flex-col  space-y-4 py-8">
            {navLinks.map((link) => (
              <li key={link.name} className="mobile-menu-item  transition-all duration-300">
                {link.submenu ? (
                  <div>
                    <button
                      onClick={() => toggleSubmenu(link.name)}
